...
### 🚀 训练
**子进程执行不阻塞 UI**，实时进度条、剩余时间、显存占用，支持手动停止（5 秒倒计时）。检测/分割走 RF-DETR，分类走 ResNet（18/34/50/101），所有网络尺寸下拉映射。支持**训练队列**：多组配置排队串行执行，可随时停止，中断或失败后可一键重新入队。
...
> 💡 训练图像尺寸：检测默认 **640**（32 的倍数）、分割默认 **648**（nano 档 12 的倍数，其余三档 24 的倍数）、分类默认 **224**。输入框右侧与悬停提示按当前档位显示可用值。
